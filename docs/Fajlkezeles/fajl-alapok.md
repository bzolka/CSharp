...
Ez így már sokkal átláthatóbb. Mostantól úgy gondolunk minden fájlra, mint bájtok egymás után következő sorozatára. Pl. amikor egy kép fájról van szó, ezekbe a számokba a kép pixeljeinek színei vannak "belekódolva". Egy egyszerű szövegfájl esetén pedig ezek a számok a szöveg egyes karaktereit jelentik (erre még visszatérünk). Amikor valamilyen programozási nyelven fájlokat írunk és olvasunk nyersen, akkor alacsony szinten bájtokat írunk ki és bájtokat olvasunk be. Pl. C# nyelven ki tudjuk írni egy bájl tömb tartalmát, vagy be tudunk olvasni fájlból részeket egy bájt tömbbe.
...
Kód és példa. Az alacsonyabb, bájtszintű fájlkezelésre ritkábban van szükség, így erre nem nézünk példákat. Mindenesetre itt van egyébként egy példa: https://docs.microsoft.com/en-us/dotnet/api/system.io.file.openwrite?view=netcore-3.1#examples. Az látszódik, hogy a `File` osztállyal nyitja meg a fájlt, és után byte tömbe teszi az adatot, és ezt írja ki a FileStream.Write műveletével (adott pozíciótól), az olvasás is bájtokkal dolgozik.
...
Próbáljuk ezt most szövegfájltként értelmezni, ennek megfelelően egy ASCII táblázatban nézzük meg minden egyes bájtra, milyen karakter tartozik hozzá (pl. itt https://hu.wikipedia.org/wiki/ASCII):

```
a l m a CR LF s z i l v a
```
...
Szövegfájlba írni a `StreamWriter` osztály `Write`, illetve `WriteLine` műveletével tudunk: a `Write` egyszerűen kiírja a fájl végére a paraméterként kapott sztringet. A `WriteLine` is kiírja a paraméterként kapott stringet, de a végére még egy sortörést is hozzáfűz ("\r\n"), így a későbbi írás már egy új sorba történik majd (ha még írunk a fájlba).
...
* \#1 - Útvonal a fájlhoz. A c:\temp mappának léteznie kell! Lényges, hogy @ karakterrel kezdjük az útvonalat, különben a `\`-t escape szekvencia kezdetének veszi, és nem `\` karakternek (vagy ha nem használjuk a @-ot, duplázzuk meg mindenhol a `\`-t, így: `\\`).
* \#2 - A `StreamWriter` egy .NET beépített osztály. Amikor létrehozunk belőle egy példányt, megnyitja a fájlt.
    * Az első paraméter (fajlUtvonal) adja meg a fájl nevét, ebbe írunk majd.
...
* \#4 - Kiírja a "szilva" szöveget, és ennek a végére is tesz sortörést. Így most nem is az eredeti feladatunknak megfelelő fájlt kapjuk, abban nem volt a szilva után sortörés! Ha ez valakit zavar, a `WriteLine` helyett a `Write`-ot haszbáljon.
...
// A {1} helyére pedig az első paramétert, vagyis a 8.2-t, és így írja ki.
...
A beépített `File` osztály statikus `WriteAllText` műveletét használjuk. Mivel ez statikus, a `File`-ból nem kell a new-val objektumot létrehozni. Csak megadjuk az útvonalat a fájlhoz és a sztringet, melynek a tartalmát egy az egyben kiírja a fájlba. Ha akarunk, megadhatunk egy harmadik paramétert is (karakterkódolás).

### Szövegfájl olvasása

 Szövegfájlból olvasni a `StreamReader` osztály `Read`, illetve `ReadLine` műveletével tudunk: a `Read` egyetlen karaktert olvas be az aktuális fájlpozíciótól, a `ReadLine` pedig az aktuális sort olvassa be egy stringbe (vagyis addig olvas az aktuális fájl pozíciótól, míg "\r\n"-t nem talál, vagy el nem ér a fájl végéig).
...
* \#3 - Beolvassa következő sort és visszatér vele, elmentjük egy sztringben.
* \#4 - Beolvassa következő sort és visszatér vele, elmentjük egy sztringben.
* \#5 - Fájl lezárása. Ez is fontos, mert ha elmarad, a fájl nyitva marad, lehet nem tudjuk más alkalmazásokból megnyitni, módosítani, törölni (ez attól függ, hogy pontosan nyitottuk meg a fájlt).
...
A példa magyarázatát a kódba fűzött megjegyzésekben látjuk. A ciklus beljesében gyakran nem csak kiírjuk a konzolra az aktuális sort, hanem fel is dolgozzuk azt. Pl., ha valamilyen szeparátottal elválasztott "elemek" vannak benne, akkor a `string.Split`-tel szét tudjuk vágni: ez egy sztring tömböt ad vissza, ebből már megfelelő konverziókkal ki tudjuk nyerni a tartalmat. Pl.
...
A `szoveg` sztringben megkapjuk a fájl teljes szöveges tartalmát egyben. Ha sortörések voltak a fájlban, azok a `szoveg` sztringben "\r\n"-ek lesznek!
Feladat: hogyan tudjuk ezt a stringet sortörések mentén vágni, és egy sztring tömbben a sorokat megkapni? (a válasz szoveg.Split("\r\n")).

Pl. itt találsz még jó magyarázatot és példákat szövegfájl írására és olvasására is: http://aries.ektf.hu/~hz/wiki7/mprog1ea/text_file
